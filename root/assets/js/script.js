...
window.addEventListener('mouseup', function (event) {
    ...
    if (event.target == trigger || (box.classList.contains('show') && event.target != trigger) && event.target != triggerSmall) {
    ...
    if (event.target == triggerSmall || (dropdown.classList.contains('show') && event.target != triggerSmall)) {
        classList(dropdown).toggle('show');
    } 
});
...
function validateForm() {
    ...
    for (let input = 0; input < document.forms['myForm'].length-1; input++) {
        classList(document.forms['myForm'][input]).remove('invalid').add('valid');
...
function classList(elt) {
    ...
    return {
        toggle: function (className) { list.toggle(className); return this; },
        add: function (className) { list.add(className); return this; },
        remove: function (className) { list.remove(className); return this; }
    };
